...
def plot_turnover(df: pd.DataFrame) -> None:
    ...
    polyfit_linear = np.polyfit(
        df.index.to_series().astype(float), df.iloc[:, -1].astype(float), deg=1
    )
    # =========================================================================
    ...
    df["capital_turnover_lin"] = np.poly1d(polyfit_linear)(
        df.index.to_series()
    )
    df["capital_turnover_exp"] = np.exp(
    ...
    plt.grid()
    plt.figure(2)
    plt.scatter(df.index, df.iloc[:, -5], label="Fixed Assets Turnover")
    plt.plot(
        df.iloc[:, [-4]],
        label="$\\hat K_{{l}} = {:.2f} {:.2f} t, R^2 = {:.4f}$".format(
            *polyfit_linear[::-1], r2_score(df.iloc[:, -5], df.iloc[:, -4])
        ),
    )
    plt.plot(
        df.iloc[:, [-3]],
        label="$\\hat K_{{e}} = \\exp ({:.2f} {:.2f} t), R^2 = {:.4f}$".format(
            *_exp[::-1], r2_score(df.iloc[:, -5], df.iloc[:, -3])
        ),
    )
    plt.title(
